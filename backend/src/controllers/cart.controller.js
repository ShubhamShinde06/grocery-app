...
export const addToCart = async (req, res) => {
  try {
    ...
    if (!shopkeeper) return res.status(404).json({ success: false, message: "Shop not found" });

    const product = await ProductModel.findById(itemId);
    if (!product) return res.status(404).json({ success: false, message: "Product not found" });

    // Check if the item already exists in the user's cart
    const existingCartItem = await cartmodel.findOne({ userId, itemId, size, finalPrice, shopId });
...
export const updateToCart = async (req, res) => {
  try {
    ...
    // Find the cart item
    const cartItem = await cartmodel.findOne({  itemId, size, quantity });
...
export const getUserCart = async (req, res) => {
  try {
    ...
    if (!cartItems.length) {
      return res.json({ success: true, cartData: [], totalCartCount: 0, message: "Cart is empty" });
    }

    // Count total cart products
    const totalCartCount = await cartmodel.countDocuments({ itemId: { $in: cartItems.map(item => item.itemId) } });
    ...
    console.error(error);
    return res.status(500).json({ success: false, message: "Server error in getUserCart" });
  }
};
...
export const deleteFromCart = async (req, res) => {
  try {
    ...
    const deletedItem = await cartmodel.findOneAndDelete({ userId, itemId, size, finalPrice, shopId });
